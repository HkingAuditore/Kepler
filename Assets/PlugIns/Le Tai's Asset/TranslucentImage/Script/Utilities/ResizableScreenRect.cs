...
namespace LeTai.Asset.TranslucentImage
{
    public static class ResizableScreenRect
    {
        ...
        private const float DRAG_EXTEND      = 16;

        private static DragHandlerDelegate currentDragHandler;

        private static readonly DragHandlerDelegate DRAG_HANDLER_CENTER =
            (Vector2 delta, ref Rect rect) => { rect.position += delta; };

        private static readonly DragHandlerDelegate DRAG_HANDLER_LEFT =
            (Vector2 delta, ref Rect rect) => { rect.xMin += delta.x; };

        private static readonly DragHandlerDelegate DRAG_HANDLER_TOP =
            (Vector2 delta, ref Rect rect) => { rect.yMin += delta.y; };

        private static readonly DragHandlerDelegate DRAG_HANDLER_BOTTOM =
            (Vector2 delta, ref Rect rect) => { rect.yMax += delta.y; };

        private static readonly DragHandlerDelegate DRAG_HANDLER_RIGHT =
            (Vector2 delta, ref Rect rect) => { rect.xMax += delta.x; };

        private static readonly DragHandlerDelegate DRAG_HANDLER_TOP_LEFT =
        ...
        private static readonly DragHandlerDelegate DRAG_HANDLER_TOP_RIGHT =
            (Vector2 delta, ref Rect rect) =>
            {
                rect.xMax += delta.x;
                rect.yMin += delta.y;
        ...
        private static readonly DragHandlerDelegate DRAG_HANDLER_BOTTOM_RIGHT =
            (Vector2 delta, ref Rect rect) =>
            {
                rect.xMax += delta.x;
                rect.yMax += delta.y;
        ...
        private static readonly DragHandlerDelegate DRAG_HANDLER_BOTTOM_LEFT =
            (Vector2 delta, ref Rect rect) =>
            {
                rect.xMin += delta.x;
                rect.yMax += delta.y;
        ...
        private static void DrawSquareFromCenter(Vector2 postion, float extent)
        {
        ...
        private static Rect HandleEvent(Rect guiRect)
        {
        ...
        private static DragHandlerDelegate ChooseDragHandler(Rect rect, Vector2 pointer)
        {
            var extend = DRAG_EXTEND * EditorGUIUtility.pixelsPerPoint;

            bool PointerXNear(float point)
            {
                return Mathf.Abs(point - pointer.x) <= extend;
            }

            bool PointerYNear(float point)
            {
                return Mathf.Abs(point - pointer.y) <= extend;
            }
        ...
        private delegate void DragHandlerDelegate(Vector2 delta, ref Rect rect);
    }
}
#endif