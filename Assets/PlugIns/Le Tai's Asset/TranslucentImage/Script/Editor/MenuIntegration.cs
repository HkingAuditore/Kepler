...
namespace LeTai.Asset.TranslucentImage.Editor
{
    public class MenuIntegration : MonoBehaviour
    {
        ...
        private static void CreateTranslucentImage(MenuCommand menuCommand)
        {
            // Create a custom game object
            var go = new GameObject("Translucent Image");
            go.AddComponent<TranslucentImage>();
            PlaceUIElementRoot(go, menuCommand);
        }

        [MenuItem("CONTEXT/Image/Convert to Translucent Image", false, 3)]
        private static void ConvertToTranslucentImage(MenuCommand menuCommand)
        {
        ...
        private static void SetPositionVisibleinSceneView(RectTransform canvasRTransform, RectTransform itemTransform)
        {
            ...
            if (RectTransformUtility.ScreenPointToLocalPointInRectangle(
                                                                        canvasRTransform,
                                                                        new Vector2(camera.pixelWidth  / 2,
