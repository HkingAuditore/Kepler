...
float3 CalculateDirectionalColor(float4 GradPos, float2 GradientYStartPos, float Rotation, float GradientHeight,
                                 float3 Color1, float3 Color2)
{
    half RotatedGrad = (GradPos.x - GradientYStartPos.x) * sin(Rotation / 57.32) + (GradPos.y - GradientYStartPos.y) *
        cos(Rotation / 57.32);
...
vertexOutput vert(vertexInput v)
{
    ...
    #else
    Maincolor = lerp(colorFront, whiteColor, 1 - dirFront) * lerp(colorBack, whiteColor, 1 - dirBack) *
        lerp(colorLeft, whiteColor, 1 - dirLeft) * lerp(colorRight, whiteColor, 1 - dirRight) *
        lerp(colorTop, whiteColor, 1 - dirTop) * lerp(colorDown, whiteColor, 1 - dirBottom);
    #endif
