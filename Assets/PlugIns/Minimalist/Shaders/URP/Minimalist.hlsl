...
vertexOutput vert(vertexInput input)
{
    ...
    #if FRONTGRADIENT
    half RotatedGrad_F = (GradPos.x - _GradientYStartPos_F.x) * sin(_Rotation_F / 57.32) + (GradPos.y -
        _GradientYStartPos_F.y) * cos(_Rotation_F / 57.32);
    half GradientFactor_F = saturate(RotatedGrad_F / - _GradientHeight_F);
    colorFront = lerp(_Color1_F, _Color2_F, GradientFactor_F);
    #endif
    #if BACKGRADIENT
    half RotatedGrad_B = (GradPos.x - _GradientYStartPos_B.x) * sin(_Rotation_B / 57.32) + (GradPos.y -
        _GradientYStartPos_B.y) * cos(_Rotation_B / 57.32);
    half GradientFactor_B = saturate(RotatedGrad_B / - _GradientHeight_B);
    colorBack = lerp(_Color1_B, _Color2_B, GradientFactor_B);
    #endif
    #if LEFTGRADIENT
    half RotatedGrad_L = (GradPos.z - _GradientYStartPos_L.x) * sin(_Rotation_L / 57.32) + (GradPos.y -
        _GradientYStartPos_L.y) * cos(_Rotation_L / 57.32);
    half GradientFactor_L = saturate(RotatedGrad_L / - _GradientHeight_L);
    colorLeft = lerp(_Color1_L, _Color2_L, GradientFactor_L);
    #endif
    #if RIGHTGRADIENT
    half RotatedGrad_R = (GradPos.z - _GradientYStartPos_R.x) * sin(_Rotation_R / 57.32) + (GradPos.y -
        _GradientYStartPos_R.y) * cos(_Rotation_R / 57.32);
    half GradientFactor_R = saturate(RotatedGrad_R / - _GradientHeight_R);
    colorRight = lerp(_Color1_R, _Color2_R, GradientFactor_R);
    #endif
    #if TOPGRADIENT
    half RotatedGrad_T = (GradPos.z - _GradientXStartPos_T.x) * cos(_Rotation_T / 57.32) + (GradPos.x -
        _GradientXStartPos_T.y) * sin(_Rotation_T / 57.32);
    half GradientFactor_T = saturate(RotatedGrad_T / - _GradientHeight_T);
    colorTop = lerp(_Color1_T, _Color2_T, GradientFactor_T);
    #endif
    #if BOTTOMGRADIENT
    half RotatedGrad_D = (GradPos.z - _GradientXStartPos_D.x) * cos(_Rotation_D / 57.32) + (GradPos.x -
        _GradientXStartPos_D.y) * sin(_Rotation_D / 57.32);
    ...
    half3 Maincolor;
    #if DONTMIX
    Maincolor = colorFront * dirFront + colorBack * dirBack + colorLeft * dirLeft + colorRight * dirRight + colorTop *
        dirTop + colorDown * dirBottom;
    #else
...
half4 frag(vertexOutput input): SV_Target
{
    ...
    #if TEXTUREMODULE_ON
    half4 _MainTexture_var = tex2D(_MainTexture, input.uv) + half4(_MainTexturePower, _MainTexturePower,
                                                                   _MainTexturePower, 0);
    ...
    //Applying AO
    #if AO_ON_UV0
    half4 AOTexVar = lerp(half4(_AOColor, 1), half4(whiteColor, 1),
                          lerp(half4(1, 1, 1, 1), tex2D(_AOTexture, input.aouv), _AOPower));
    Result *= AOTexVar;
    #elif AO_ON_UV1
    ...
    Result = half4(lerp(Result.r, maxVal, 1 - _Saturation), lerp(Result.g, maxVal, 1 - _Saturation),
                   lerp(Result.b, maxVal, 1 - _Saturation), Result.a);
    #endif
