...
namespace Minimalist
{
    public class EditorGUILayoutExtended : Editor
    {
        ...
        {
            var controlId = GUIUtility.GetControlID("EditorTextField".GetHashCode(), FocusType.Keyboard, _position);
...
//	}
        ...
        {
            var totalRect = EditorGUILayout.GetControlRect(_options);
//		float width;
...
//		else width = Mathf.Clamp(_labelwidth, totalRect.width * 0.2f, totalRect.width * 0.8f);
            ...
            var labelRect = new Rect(totalRect.x, totalRect.y, _labelwidth, totalRect.height);
            var inputRect = new Rect(totalRect.x + _labelwidth, totalRect.y, totalRect.width - _labelwidth,
                                     totalRect.height);
