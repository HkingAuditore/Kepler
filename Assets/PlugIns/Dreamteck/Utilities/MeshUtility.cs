...
namespace Dreamteck
{
    public class MeshUtility
    {
        ...
        public static int[] GeneratePlaneTriangles(int x, int z, bool flip, int startTriangleIndex = 0,
                                                   int startVertex = 0)
        {
            var nbFaces   = x * (z - 1);
        ...
        public static int[] GeneratePlaneTriangles(ref int[] triangles,              int x, int z, bool flip,
                                                   int       startTriangleIndex = 0, int startVertex = 0,
                                                   bool      reallocateArray    = false)
        {
            ...
            if (reallocateArray && triangles.Length != nbFaces * 6)
            {
                if (startTriangleIndex > 0)
                {
                    ...
                    for (var i = 0; i < startTriangleIndex; i++) newTris[i] = triangles[i];
                    triangles = newTris;
                }
                else
                {
            ...
            var g = x + 1;
            var t = startTriangleIndex;
            for (var face = 0; face < nbFaces + z - 2; face++)
            {
                if ((face + 1) % (float) g == 0f && face != 0) face++;
                if (flip)
                {
        ...
        public static void CalculateTangents(TS_Mesh mesh)
        {
            ...
            {
                var i1 = mesh.triangles[tri];
                var i2 = mesh.triangles[tri + 1];
                var i3 = mesh.triangles[tri + 2];

                var x1 = mesh.vertices[i2].x - mesh.vertices[i1].x;
                var x2 = mesh.vertices[i3].x - mesh.vertices[i1].x;
                var y1 = mesh.vertices[i2].y - mesh.vertices[i1].y;
                var y2 = mesh.vertices[i3].y - mesh.vertices[i1].y;
                var z1 = mesh.vertices[i2].z - mesh.vertices[i1].z;
                var z2 = mesh.vertices[i3].z - mesh.vertices[i1].z;

                var s1 = mesh.uv[i2].x - mesh.uv[i1].x;
                var s2 = mesh.uv[i3].x - mesh.uv[i1].x;
                var t1 = mesh.uv[i2].y - mesh.uv[i1].y;
                var t2 = mesh.uv[i3].y - mesh.uv[i1].y;
        ...
        public static void MakeDoublesided(Mesh input)
        {
            ...
            for (var i = 0; i < input.subMeshCount; i++) submeshes.Add(input.GetTriangles(i));
            ...
            for (var i = 0; i < submeshes.Count; i++)
            {
                ...
                {
                    var index1 = submeshes[i][n];
                    var index2 = submeshes[i][n + 1];
                    var index3 = submeshes[i][n + 2];
            ...
            for (var i = 0; i < newSubmeshes.Count; i++) input.SetTriangles(newSubmeshes[i], i);
        }
        ...
        {
            var vertexHalf = input.vertices.Length  / 2;
            var trisHalf   = input.triangles.Length / 2;
        ...
        {
            for (var i = 0; i < vertices.Length; i++) vertices[i] = transform.TransformPoint(vertices[i]);
        }
        ...
        {
            for (var i = 0; i < normals.Length; i++) normals[i] = transform.TransformDirection(normals[i]);
        }

        public static void InverseTransformNormals(Vector3[] normals, Transform transform)
        {
            for (var i = 0; i < normals.Length; i++) normals[i] = transform.InverseTransformDirection(normals[i]);
        }
        ...
        public static string ToOBJString(Mesh mesh, Material[] materials)
        {
            ...
            sb.Append("\n");
            foreach (var n in mesh.normals) sb.Append(string.Format("vn {0} {1} {2}\n", -n.x, n.y, n.z));
            sb.Append("\n");
            foreach (Vector3 v in mesh.uv) sb.Append(string.Format("vt {0} {1}\n", v.x, v.y));
            sb.Append("\n");
            foreach (var v in mesh.uv2) sb.Append(string.Format("vt2 {0} {1}\n", v.x, v.y));
            sb.Append("\n");
            foreach (var v in mesh.uv3) sb.Append(string.Format("vt2 {0} {1}\n", v.x, v.y));
            sb.Append("\n");
            foreach (var c in mesh.colors) sb.Append(string.Format("vc {0} {1} {2} {3}\n", c.r, c.g, c.b, c.a));
        ...
        public static Mesh Copy(Mesh input)
        {
            ...
            for (var i = 0; i < input.subMeshCount; i++) copy.SetTriangles(input.GetTriangles(i), i);
            return copy;
        }
        ...
        public static void FlipFaces(TS_Mesh input)
        {
            for (var i = 0; i < input.subMeshes.Count; i++)
            {
                var array = input.subMeshes[i];
                FlipTriangles(ref array);
            }

            FlipTriangles(ref input.triangles);
            for (var i = 0; i < input.normals.Length; i++) input.normals[i] *= -1f;
        }
        ...
        public static void BreakMesh(Mesh input, bool keepNormals = true)
        {
            ...
            var oldVertices    = input.vertices;
            var oldUvs         = input.uv;
            var oldNormals     = input.normals;
            var oldTangents    = input.tangents;
            var oldColors      = input.colors;
            var oldBoneWeights = input.boneWeights;

            if (oldColors.Length != oldVertices.Length)
            {
                oldColors = new Color[oldVertices.Length];
                for (var i = 0; i < oldColors.Length; i++) oldColors[i] = Color.white;
            }

            var submeshList = new List<int[]>();
            var submeshes   = input.subMeshCount;
            var vertIndex   = 0;
            for (var i = 0; i < submeshes; i++)
            {
                var submesh = input.GetTriangles(i);
            ...
            for (var i = 0; i < submeshList.Count; i++) input.SetTriangles(submeshList[i], i);
        }
