...
namespace Dreamteck.Splines.Editor
{
    ...
    {
        private MeshDefinitionWindow    definitionWindow;
        private SplineMesh.Channel      renameChannel;
        private MeshScaleModifierEditor scaleModifierEditor;
                                                ...
                                                "Yes", "No"))
                {
                    if (rend.sharedMaterial == AssetDatabase.GetBuiltinExtraResource<Material>("Default-Diffuse.mat"))
                        if (EditorUtility.DisplayDialog("Replace Material",
                                                        "The renderer is using the default material. Replace it?",
                                                        "Yes", "No"))
        ...
        private void OnDuplicateChannel(object index)
        {
            ...
            var source     = extruder.GetChannel((int) index);
            var newChannel = extruder.AddChannel(source.name);
                    ...
                    if (Event.current.type == EventType.MouseDown)
                    {
                        ...
                        if (Event.current.button == 0)
                        {
                            ...
                            else
                            {
                                selectedChannel = i;
                                scaleModifierEditor =
                                    new MeshScaleModifierEditor(user, this, user.GetChannel(i).scaleModifier);
        ...
        {
            var channel = ((SplineMesh) target).GetChannel(channelIndex);
            var open    = selectedChannel == channelIndex;

            GUILayout.BeginVertical(EditorStyles.helpBox);
            if (renameChannel == channel && Event.current.type == EventType.KeyDown &&
            ...
            for (var i = 0; i < channel.GetMeshCount(); i++) MeshRow(channel, i);
            Object obj = null;
            ...
            {
                channel.minRotation =
                    EditorGUILayout.Vector3Field(channel.randomRotation ? "Rotation Min" : "Rotation",
                                                 channel.minRotation);
                if (channel.randomRotation)
            ...
            if (channel.type == SplineMesh.Channel.Type.Extrude)
            {
                var lastZ = channel.minScale.z;
                var scale = channel.minScale;
        ...
        {
            var definition = channel.GetMesh(index);
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(15);
        ...
        public class MeshDefinitionWindow : EditorWindow
        {
            ...
            private void OnGUI()
            {
                ...
                definition.mirror =
                    (SplineMesh.Channel.MeshDefinition.MirrorMethod)
                    EditorGUILayout.EnumPopup("Mirror", definition.mirror);
