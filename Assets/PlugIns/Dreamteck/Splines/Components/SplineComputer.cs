...
namespace Dreamteck.Splines
{
    ...
    public class SplineComputer : MonoBehaviour
    {
        ...
        [HideInInspector] public bool multithreaded;

        [HideInInspector] public bool rebuildOnAwake;
        ...
        [HideInInspector] [SerializeField] private Spline spline = new Spline(Spline.Type.CatmullRom);

        [HideInInspector] [SerializeField] private SplineSample[] _rawSamples = new SplineSample[0];

        [HideInInspector] [SerializeField] private SplineSample[] _transformedSamples = new SplineSample[0];

        [HideInInspector] [SerializeField] private SampleCollection sampleCollection = new SampleCollection();

        [HideInInspector] [SerializeField] private double[] originalSamplePercents = new double[0];

        [HideInInspector] [SerializeField] private int _sampleCount;

        [HideInInspector] [SerializeField] private bool _is2D;

        [HideInInspector] [SerializeField] private bool hasSamples;

        [HideInInspector] [SerializeField] private bool[] pointsDirty = new bool[0];
            ...
            set
            {
                if (value != _space)
                {
                    ...
                    if (_space == Space.Local)
                    {
                        ...
                        for (var i = 0; i < _transformedSamples.Length; i++)
        ...
        {
            get => spline.linearAverageDirection;
        ...
        public float optimizeAngleThreshold
        {
            ...
            set
            {
                if (value != _optimizeAngleThreshold)
                {
                    ...
                    if (_sampleMode == SampleMode.Optimized) Rebuild(true);
                }
            }
        }
        ...
        {
            var transformChanged = TransformHasChanged();
            if (transformChanged)
            {
                ResampleTransform();
        ...
        {
            if (!IsSubscribed(input)) ArrayUtility.Add(ref _subscribers, input);
        }
        ...
        {
            var points = new SplinePoint[spline.points.Length];
        ...
        {
            var newPoints = new SplinePoint[spline.points.Length + count];
        ...
        {
            var percent = DMath.Clamp01((double) pointIndex / (pointCount - 1));
            if (_sampleMode != SampleMode.Uniform) return percent;

            if (originalSamplePercents.Length <= 1) return 0.0;
            for (var i = originalSamplePercents.Length - 2; i >= 0; i--)
                if (originalSamplePercents[i] < percent)
                {
                    var inverseLerp =
                        DMath.InverseLerp(originalSamplePercents[i], originalSamplePercents[i + 1], percent);
                    return DMath.Lerp(sampleCollection.samples[i].percent, sampleCollection.samples[i + 1].percent,
                                      inverseLerp);
                }
        ...
        /// <returns></returns>
        public void Project(SplineSample result, Vector3 position, double from = 0.0, double to = 1.0,
                            EvaluateMode mode = EvaluateMode.Cached, int subdivisions = 4)
        {
            if (mode == EvaluateMode.Calculate)
            {
                position = InverseTransformPoint(position);
                var percent = spline.Project(position, subdivisions, from, to);
                spline.Evaluate(result, percent);
                TransformResult(result);
                return;
            }
            ...
            for (var i = _subscribers.Length - 1; i >= 0; i--)
                if (_subscribers[i] != null)
                {
                    if (_subscribers[i].spline != this)
                        ArrayUtility.RemoveAt(ref _subscribers, i);
                    else if (_subscribers[i].isActiveAndEnabled) _subscribers[i].Rebuild();
                }
                else
                {
                    ArrayUtility.RemoveAt(ref _subscribers, i);
        ...
        private void UnsetPointsDirty()
        {
            ...
            for (var i = 0; i      < pointsDirty.Length; i++) pointsDirty[i] = false;
        }

        private void SetPointsDirty()
        {
            if (pointsDirty.Length != spline.points.Length) pointsDirty      = new bool[spline.points.Length];
            for (var i = 0; i      < pointsDirty.Length; i++) pointsDirty[i] = true;
        }
        ...
        private void CalculateSamples()
        {
            ...
            else
            {
                ...
                if (_rawSamples.Length != spline.iterations)
                {
                    ...
                    for (var i = 0; i < _rawSamples.Length; i++) _rawSamples[i] = new SplineSample();
                }
                ...
                {
                    var samplePercent = (double) i / (_rawSamples.Length - 1);
        ...
        private void TransformSamples(bool forceTransformAll = false)
        {
            ...
            if (space == Space.Local)
                for (var i = 0; i < _rawSamples.Length; i++)
                {
                    if (!forceTransformAll && isHermite
                        ? !IsDirtyHermite(_rawSamples[i].percent)
                        : !IsDirtyBezier(_rawSamples[i].percent)) continue;
            ...
            if (_sampleMode == SampleMode.Optimized)
            {
                ...
                sampleCollection.optimizedIndices[sampleCollection.optimizedIndices.Length - 1] =
                    sampleCollection.Count - 1;
                for (var i = 1; i < _rawSamples.Length - 1; i++)
                {
                    sampleCollection.optimizedIndices[i] = 0;
                    var samplePercent = (double) i / (_rawSamples.Length - 1);
                    for (var j = 0; j < sampleCollection.Count; j++)
                    {
        ...
        private void OptimizeSamples()
        {
            ...
            _sampleCount = 2;
            var lastSavedDirection = _transformedSamples[0].forward;
            sampleFlter[0]                      = true;
            sampleFlter[sampleFlter.Length - 1] = true; //Always include the first and last samples
            for (var i = 1; i < _transformedSamples.Length - 1; i++)
            {
                var angle = Vector3.Angle(lastSavedDirection, _transformedSamples[i].forward);
            ...
            if (sampleCollection.Count != _sampleCount || sampleCollection.samples == _transformedSamples)
            {
                ...
                for (var i = 0; i < sampleCollection.Count; i++) sampleCollection.samples[i] = new SplineSample();
            }
        ...
        {
            var pointValue = (float) samplePercent * (pointCount - 1);
        ...
        private bool IsDirtyHermite(double samplePercent)
        {
            ...
            var backwardIndex = previousIndex - 1;
            if (backwardIndex < 0)
            {
                               ...
                               QueryTriggerInteraction hitTriggers = QueryTriggerInteraction.UseGlobal)
        {
            ...
            while (true)
            {
                ...
                percent = DMath.Move(percent, to, moveStep / resolution);
                var toPos = EvaluatePosition(percent);
                var h = Physics.RaycastAll(fromPos, toPos - fromPos, Vector3.Distance(fromPos, toPos), layerMask,
                                           hitTriggers);
        ...
        {
            for (var i = 0; i < triggerGroups.Length; i++) triggerGroups[i].Check(start, end);
        }
        ...
        public void ResetTriggers(int group)
        {
            ...
            for (var i = 0; i < triggerGroups[group].triggers.Length; i++) triggerGroups[group].triggers[i].Reset();
        }
        ...
        public Dictionary<int, List<Node.Connection>> GetJunctions(double start = 0.0, double end = 1.0)
        {
            ...
            var startValue = (pointCount - 1) * (float) start;
            var endValue   = (pointCount - 1) * (float) end;
            for (var i = 0; i < nodes.Length; i++)
            {
                var add = false;
        ...
        public void GetConnectedComputers(List<SplineComputer> computers, List<int> connectionIndices,
                                          List<int> connectedIndices, double percent, Spline.Direction direction,
                                          bool includeEqual)
        {
            ...
            var pointValue = Mathf.FloorToInt((pointCount - 1) * (float) percent);
            for (var i = 0; i < nodes.Length; i++)
            {
                var condition = false;
                if (includeEqual)
        ...
        private void GetConnectedComputers(ref List<SplineComputer> computers)
        {
            ...
            for (var i = 0; i < comp.nodes.Length; i++)
            {
                ...
                var connections = comp.nodes[i].node.GetConnections();
        ...
        private void RemoveNodeLinkAt(int index)
        {
            ...
            var newLinks = new NodeLink[nodes.Length - 1];
            for (var i = 0; i < nodes.Length; i++)
                if (i      == index) continue;
        ...
        private void UpdateConnectedNodes(SplinePoint[] worldPoints)
        {
            for (var i = 0; i < nodes.Length; i++)
            {
                ...
                var found = false;
                foreach (var connection in nodes[i].node.GetConnections())
        ...
        {
            [SerializeField] internal Node node;

            [SerializeField] internal int pointIndex;
        ...
        [HideInInspector] public bool alwaysDraw;

        [HideInInspector] public bool drawThinckness;

        [HideInInspector] public bool billboardThickness = true;
