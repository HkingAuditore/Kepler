...
namespace Dreamteck.Splines.Editor
{
    public class SplineSampleModifierEditor : SplineUserSubEditor
    {
        ...
        public SplineSampleModifierEditor(SplineUser user, SplineUserEditor editor, SplineSampleModifier input) :
            base(user, editor)
        {
            module = input;
            title  = "Tracer Module";
        ...
        public override void DrawInspector()
        {
            ...
            for (var i = 0; i < keys.Count; i++)
            {
                ...
                {
                    EditorGUILayout.LabelField(i + " [" + Mathf.Round((float) keys[i].start * 10) / 10f + " - " +
                                               Mathf.Round((float) keys[i].end              * 10) / 10f + "]");
                }
        ...
        protected virtual void KeyHandles(SplineSampleModifier.Key key, bool edit)
        {
            ...
            double value   = 0f;
            value = key.start;
            SplineComputerEditorHandles.Slider(user.spline, ref value, user.spline.editorPathColor, "Start",
            ...
            value = key.end;
            SplineComputerEditorHandles.Slider(user.spline, ref value, user.spline.editorPathColor, "End",
