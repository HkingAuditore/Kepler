...
namespace Dreamteck.Splines
{
    ...
    public class SplineFollower : SplineTracer
    {
        ...
        [HideInInspector] public FollowMode followMode = FollowMode.Uniform;
        ...
        /// </summary>
        [HideInInspector] public bool preserveUniformSpeedWithOffset;

        [SerializeField] [HideInInspector] private float _followSpeed = 1f;

        [SerializeField] [HideInInspector] private float _followDuration = 1f;

        [SerializeField] [HideInInspector] [Range(0f, 1f)]
        private double _startPosition;

        [SerializeField] [HideInInspector] private FollowerSpeedModifier _speedModifier = new FollowerSpeedModifier();

        [SerializeField] [HideInInspector] private FloatEvent _unityOnEndReached;

        [SerializeField] [HideInInspector] private FloatEvent _unityOnBeginningReached;
        ...
        private void Follow()
        {
            ...
            switch (followMode)
            {
                case FollowMode.Uniform:
                    var speed = _followSpeed + _speedModifier.GetSpeed(result);
        ...
        public override void SetDistance(float distance, bool checkTriggers = false, bool handleJuncitons = false)
        {
            ...
            if (samplesAreLooped && clipFrom == clipTo && distance > 0f && lastClippedPercent == 0.0)
        ...
        public void Move(double percent)
        {
            ...
            {
                if (startPercent > 0.0) callOnBeginningReached = true;
                switch (wrapMode)
                {
                    case Wrap.Default:
                        p = 0.0;
            ...
            if (callOnEndReached)
            {
                if (onEndReached != null) onEndReached(startPercent);
                _unityOnEndReached.Invoke((float) startPercent);
            }
            else if (callOnBeginningReached)
            {
                if (onBeginningReached != null) onBeginningReached(startPercent);
        ...
        public void Move(float distance)
        {
            ...
            if (direction == Spline.Direction.Forward)
            {
                ...
                {
                    if (startPercent < 1.0) endReached = true;
                    switch (wrapMode)
                    {
                        case Wrap.Loop:
            ...
            else
            {
                ...
                {
                    if (startPercent > 0.0) beginningReached = true;
                    switch (wrapMode)
                    {
                        case Wrap.Loop:
                            _result.percent = DoTravel(1.0, distance - moved, out moved);
