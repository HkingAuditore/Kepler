...
namespace Dreamteck.Splines
{
    ...
    public class Spline
    {
        ...
        public float CalculateLength(double from = 0.0, double to = 1.0, double resolution = 1.0)
        {
            ...
            if (to < from) to = from;
            var percent       = from;
            var lastPos       = EvaluatePosition(percent);
            var sum           = 0f;
            while (true)
            {
                percent = DMath.Move(percent, to, moveStep / resolution);
                var pos = EvaluatePosition(percent);
        ...
        public double Project(Vector3 position, int subdivide = 4, double from = 0.0, double to = 1.0)
        {
            ...
            {
                var closest = GetClosestPoint(subdivide, position, from, to,
                                              Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
                if (closest < moveStep)
                {
                    var nextClosest = GetClosestPoint(subdivide, position, 0.5, to,
                                                      Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
                    if (Vector3.Distance(position, EvaluatePosition(nextClosest)) <
                        Vector3.Distance(position, EvaluatePosition(closest))) return nextClosest;
                }
        ...
        public void Evaluate(SplineSample result, double percent)
        {
            ...
            var doubleIndex = (points.Length - 1) * percent;
            var pointIndex  = Mathf.Clamp(DMath.FloorInt(doubleIndex), 0, points.Length - 2);
            var getPercent  = doubleIndex - pointIndex;
            var point       = EvaluatePosition(percent);
        ...
        public void Evaluate(ref SplineSample[] samples, double from = 0.0, double to = 1.0)
        {
            ...
            var fromValue         = from * (iterations - 1);
            var toValue           = to   * (iterations - 1);
            var clippedIterations = DMath.CeilInt(toValue) - DMath.FloorInt(fromValue) + 1;
        ...
        /// <param name="to">Target position [from-1]</param>
        /// <returns></returns>
        public void EvaluateUniform(ref SplineSample[] samples, ref double[] originalSamplePercents, double from = 0.0,
                                    double             to = 1.0)
        {
            if (points.Length == 0)
            {
            ...
            var lengthStep = CalculateLength(from, to) / (iterations - 1);
            Evaluate(samples[0], from);
            samples[0].percent = originalSamplePercents[0] = from;
            var lastPercent                                = from;
            var moved                                      = 0f;
        ...
        public double Travel(double start, float distance, out float moved, Direction direction)
        {
            ...
            if (direction == Direction.Backward && start <= 0.0) return 0.0;
            ;
            ...
            var i               = iterations - 1;
            var nextSampleIndex = direction == Direction.Forward ? DMath.CeilInt(start * i) : DMath.FloorInt(start * i);
            var lastDistance    = 0f;
            var percent         = start;
            while (true)
            {
        ...
        public void EvaluatePosition(ref Vector3 point, double percent)
        {
            percent = DMath.Clamp01(percent);
            var doubleIndex                     = (points.Length - 1) * percent;
            var pointIndex                      = DMath.FloorInt(doubleIndex);
        ...
        private double GetClosestPoint(int iterations, Vector3 point, double start, double end, int slices)
        {
            if (iterations <= 0)
            {
                var startDist = (point - EvaluatePosition(start)).sqrMagnitude;
                var endDist   = (point - EvaluatePosition(end)).sqrMagnitude;
        ...
        public void Break(int at)
        {
            ...
            var prev                                                                   = new SplinePoint[at];
            for (var i = 0; i  < prev.Length; i++) prev[i]                             = points[i];
            for (var i = at; i < points.Length - 1; i++) points[i                - at] = points[i];
            for (var i = 0; i  < prev.Length; i++) points[points.Length - at + i - 1]  = prev[i];
            ...
            switch (type)
            {
                ...
                case Type.CatmullRom:
                    for (var i = 0; i < points.Length; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            var direction = catPoints[1] - catPoints[2];
                            if (closed)
                            {
                        ...
                        {
                            var direction = catPoints[2]                    - catPoints[3];
                            points[i].SetTangentPosition(points[i].position + direction / 3f);
                        }
                        else
                        {
                            var direction = catPoints[0]                    - catPoints[2];
                            points[i].SetTangentPosition(points[i].position + direction / 6f);
                        }
                    }
        ...
        private void BSPGetPoint(ref Vector3 point, double time, int i)
        {
            ...
            if (points.Length > 1)
            {
                var t1 = (float) DMath.Clamp01(time);
                GetCatPoints(i);
        ...
        private void CatmullRomGetPoint(ref Vector3 point, double t, int i)
        {
            ...
            if (points.Length > 1)
            {
                ...
                point = 0.5f * (2f * catPoints[1] + (-catPoints[0] + catPoints[2]) * t1
                                                  + (2f * catPoints[0] - 5f * catPoints[1] + 4f * catPoints[2] -
                                                     catPoints[3]) * t2
                                                  + (-catPoints[0] + 3f * catPoints[1] - 3f * catPoints[2] +
                                                     catPoints[3]) * t3);
            }
        }
        ...
        private void GetCatPoints(int i)
        {
            ...
            if (i > 0) catPoints[0] = points[i - 1].position;
                ...
                catPoints[0] = points[i].position + (points[i].position - points[i + 1].position); //Extrapolate
