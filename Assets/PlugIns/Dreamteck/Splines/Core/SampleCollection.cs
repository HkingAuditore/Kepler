...
namespace Dreamteck.Splines
{
    ...
    {
        [HideInInspector] public SplineSample[] samples = new SplineSample[0];
        ...
        //legacy properties
        [HideInInspector] public double clipFrom, clipTo = 1.0;

        [HideInInspector] public bool loopSamples;
        ...
        public int GetClippedSampleCount(double clipFrom, double clipTo, out int startIndex, out int endIndex)
        {
            ...
            {
                var toSamples   = endIndex + 1;
                var fromSamples = Count    - startIndex;
        ...
        public void GetSamplingValues(double percent, out int sampleIndex, out double lerp)
        {
            ...
            {
                var indexValue = percent * (optimizedIndices.Length - 1);
                var index      = DMath.FloorInt(indexValue);
                ...
                if (index < optimizedIndices.Length - 1)
                {
                    ...
                    var indexLerp              = indexValue - index;
                    var sampleIndexPercent     = (double) index       / (optimizedIndices.Length - 1);
                    var nextSampleIndexPercent = (double) (index + 1) / (optimizedIndices.Length - 1);
        ...
        public void Evaluate(ref SplineSample[] results, double from = 0.0, double to = 1.0)
        {
            ...
            var clippedIterations = toIndex - fromIndex + 1;
            ...
            for (var i = 1; i < results.Length - 1; i++) results[i] = samples[i + fromIndex];
        }
        ...
        public void EvaluatePositions(ref Vector3[] positions, double from = 0.0, double to = 1.0)
        {
            ...
            for (var i = 1; i < positions.Length - 1; i++) positions[i] = samples[i + fromIndex].position;
        }
        ...
        public double Travel(double start,          float  distance, Spline.Direction direction, out float moved,
                             double clipFrom = 0.0, double clipTo = 1.0)
        {
            moved = 0f;
            if (!hasSamples) return 0.0;
            if (direction == Spline.Direction.Forward  && start >= 1.0) return clipTo;
            if (direction == Spline.Direction.Backward && start <= 0.0) return clipFrom;

            var lastPercent = start;
            if (distance == 0f) return lastPercent;
            var    lastPos = EvaluatePosition(start);
            int    sampleIndex;
            double lerp;
            ...
            var samplesAreLooped = clipTo < clipFrom;
            ...
            var p = DMath.Lerp(lastPercent, samples[sampleIndex].percent, 1f - moveExcess / lastDistance);
            moved -= moveExcess;
            return p;
        }
        ...
        public double TravelWithOffset(double    start, float  distance, Spline.Direction direction, Vector3 offset,
                                       out float moved, double clipFrom = 0.0, double clipTo = 1.0)
        {
            moved = 0f;
            if (!hasSamples) return 0.0;
            if (direction == Spline.Direction.Forward  && start >= 1.0) return clipTo;
            ...
            var lastPos = _workSample.position + _workSample.up * (offset.y * _workSample.size) +
                          _workSample.right                     * (offset.x * _workSample.size) +
                          _workSample.forward                   * (offset.z * _workSample.size);
            ...
            while (moved < distance)
            {
                var newPos = samples[sampleIndex].position                                         +
                ...
                if (direction == Spline.Direction.Forward)
                {
                    if (sampleIndex == Count - 1)
                    {
                        if (samplesAreLooped)
                        {
                            lastPos = samples[0].position + samples[0].up * (offset.y * samples[0].size) +
        ...
        /// <returns></returns>
        public void Project(Vector3 position, int controlPointCount, SplineSample result, double from = 0.0,
                            double  to = 1.0)
        {
            ...
            var steps = (controlPointCount - 1) *
                        6; //Sampling six points per segment is enough to find the closest point range
            var step           = Count / steps;
            if (step < 1) step = 1;
            var    minDist     = (position - samples[0].position).sqrMagnitude;
            var    fromIndex   = 0;
            var    toIndex     = Count - 1;
            double lerp;
            ...
            for (var i = fromIndex; i <= toIndex; i += step)
            {
                ...
                var dist           = (position - samples[i].position).sqrMagnitude;
                if (dist < minDist)
                {
                    minDist   = dist;
            ...
            for (var i = checkFrom + 1; i <= checkTo; i++)
            {
                var dist = (position - samples[i].position).sqrMagnitude;
                if (dist < minDist)
                {
            ...
            var back = LinearAlgebraUtility.ProjectOnLine(samples[backIndex].position, samples[index].position,
                                                          position);
            var front = LinearAlgebraUtility.ProjectOnLine(samples[index].position, samples[frontIndex].position,
                                                           position);
            var backLength       = (samples[index].position - samples[backIndex].position).magnitude;
            var frontLength      = (samples[index].position - samples[frontIndex].position).magnitude;
            var backProjectDist  = (back                    - samples[backIndex].position).magnitude;
            var frontProjectDist = (front                   - samples[frontIndex].position).magnitude;
            ...
            {
                var projected =
                    LinearAlgebraUtility.ProjectOnLine(samples[Count - 1].position, samples[Count - 2].position,
                                                       position);
                if ((position - projected).sqrMagnitude < (position - result.position).sqrMagnitude)
                {
                    double l = LinearAlgebraUtility.InverseLerp(samples[Count - 1].position,
                                                                samples[Count - 2].position, projected);
        ...
        public float CalculateLengthWithOffset(Vector3 offset, double from = 0.0, double to = 1.0)
        {
            ...
            var pos = _workSample.position + _workSample.up * (offset.y * _workSample.size) +
                      _workSample.right                     * (offset.x * _workSample.size) +
                      _workSample.forward                   * (offset.z * _workSample.size);
            int    fromIndex, toIndex;
            double lerp;
            ...
            for (var i = fromIndex + 1; i < toIndex; i++)
            {
                var newPos = samples[i].position + samples[i].up * (offset.y * samples[i].size) +
            ...
            _workSample.position += _workSample.up      * (offset.y * _workSample.size) +
                                    _workSample.right   * (offset.x * _workSample.size) +
                                    _workSample.forward * (offset.z * _workSample.size);
