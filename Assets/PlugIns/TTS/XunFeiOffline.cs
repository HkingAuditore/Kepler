...
public class XunFeiOffline : MonoBehaviour
{
    ...
    {
        string xiaoyan_path = (Application.streamingAssetsPath + "PlugIns/TTS/xiaoyan.jet").Replace("/", "\\");
        string common_path = (Application.streamingAssetsPath + "PlugIns/TTS/common.jet").Replace("/", "\\");
    ...
    {
        int message = MSCDLL.MSPLogin(string.Empty, string.Empty, "appid=55b70993,word_dir= . ");
    ...
    private void Online_TTS(string speekText)
    {
        ...
        //语音合成设置文本
        err_code = MSCDLL.QTTSTextPut(Marshal.PtrToStringAnsi(session_id), speekText, (uint)Encoding.Default.GetByteCount(speekText), string.Empty);
        if (err_code != (int)Errors.MSP_SUCCESS)
        {
        ...
        while (true)
        {
            IntPtr source = MSCDLL.QTTSAudioGet(Marshal.PtrToStringAnsi(session_id), ref audio_len, ref synth_status, ref err_code);
