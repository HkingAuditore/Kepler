...
public class OrbitPanelUI : MonoBehaviour
{
    ...
    public void Init()
    {
        ...
        try
        {
            this.orbit          = _gravityTracing.GetConicSection(astralBody);
            this.isConicSection = true;
        }
        catch (Exception e)
        {
            this.isConicSection = false;
        }
        

        if (this.isConicSection && !float.IsNaN(orbit.semiMajorAxis) && !float.IsNaN(orbit.semiMinorAxis))
        {
            ...
            orbitGraphUI.Init();
            _gravityTracing.DrawMathOrbit(this.orbit, 20);
        }
        else
        {
